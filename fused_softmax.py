...
def naive_softmax(x: torch.Tensor):
    ...
    # 由于在DRAM的来回读写会带来额外，操作， 使用kernel fusion就可以让这些操作
    # 全部就在chip上完成。
    return ret
...
def softmax_kernel(output_ptr, input_ptr, input_row_stride,
    ...
    for row_idx in tl.range(row_start, n_rows, row_step, num_stages=num_stages):
        ...
        # input_row_stride 是从当前行首元素跳到下一行首元素时，指针需要前进多少个元素（或字节）。
        # 由于实际GPU处理的时候都是会对齐到2的次数，所以不一定是列的数量
        col_offsets = tl.arange(0, BLOCK_SIZE)
        input_ptrs = row_start_ptr + col_offsets
        # 因为 这段 kernel 设计时就把 BLOCK_SIZE 选成“至少覆盖整行”的长度，所以只需要一次
...
benchmark.run(print_data=True, show_plots=True, save_path=results/ "fused_softmax")
